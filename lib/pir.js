...
var PIR = function (options) {
  ...
  setInterval(function () {
    this.board.digitalRead(this.pin, function(err, data) {
      // If this is not a calibration event
      if (self.state != null && self.state != +data) {
        ...
        if (data === '01') {
          self.emit('motionstart', err, timestamp);
        }

        // 'motionend' event fired when motion has ceased
        // within the observable range of the PIR sensor
        if (data === '00') {
          self.emit('motionend', err, timestamp);
        }
      }
      ...
      if (!self.calibrated) {
        ...
        self.state = +data;
        self.emit('calibrated', err, timestamp);
      }
    });
  }.bind(this), 50);
